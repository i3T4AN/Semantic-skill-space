...
def main() -> None:
    ...
    for idx, case in enumerate(CASES, start=1):
        ...
        grade = grade_answer(answer=answer, expected=expected, skill_text="")
    ...
    out_path = out_dir / "c0_results.json"
    payload = build_results_payload(results)
    out_path.write_text(json.dumps(payload, indent=2))

    avg_latency = total_time / len(CASES)
    print(f"C0 score: {total_score:.1f}/{len(CASES)}")
    print(f"C0 avg latency: {avg_latency:.4f} sec/question")
